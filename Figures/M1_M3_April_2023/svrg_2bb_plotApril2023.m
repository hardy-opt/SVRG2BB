...
pathh=strcat('SVRG_BB/Results_July2022/',darg,'/');
...
method = {'svrg_2bbs_eta_one','svrg_2bbs_eta_constant','svrg_2bbs_eta_decay_m1','LBFGS'};%'svrg_2bbs_eta_constant'
...
for j = 1:l
    ...
    g{j}.time = mean(d{j}.S1.otime,2);
    
    g{j}.time = g{j}.time(1:ep);
    
    g{j}.epoch = d{j}.S1.epoch(1:ep);
    
    
    % Variance
    g{j}.variance = mean(d{j}.S1.variance,2);
    
    g{j}.variance = g{j}.variance(1:ep);
        
    % Train cost
    g{j}.cost_mean = mean(d{j}.S1.ocost(1:ep,:),2);
    
    
    g{j}.cost_std = isbar*std(d{j}.S1.ocost(1:ep,:),[],2);
...
%     g{j}.optgap = abs(g{j}.opt_mean);% - f_opt + epsilon)/(1+f_opt);
    ...
    % Validation cost
    g{j}.vcost_mean = mean(d{j}.S1.vcost(1:ep,:),2);
    g{j}.vcost_std = (isbar*std(d{j}.S1.vcost(1:ep,:),[],2));
