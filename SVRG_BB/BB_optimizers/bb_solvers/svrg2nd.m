function [w, infos] = svrg2nd(problem, in_options)
...
% Originally Created by H. Tankaria
    ...
    if options.verbose > 0
        fprintf('SVRG 2nd: Epoch = %03d, cost = %.16e, optgap = %.4e\n', epoch, f_val, optgap);
    end      
        ...
        for j = 1 : num_of_bachces
            ...
            vec = w - w0;
                ...
                if n<d
                        ...
                        Ht = problem.partial_hess(w0,indice_j);  %% Stochastic Hessian at w0 (x^m)
                        Htk = problem.partial_hess_vec(vec,indice_j,Ht);
                elseif d<n
                        Hv = H*vec;
                        Ht = problem.hess(w0,indice_j); 
                        Htk = Ht*vec;
                end
...
%         
        ...
        % display infos
        if options.verbose > 0
            fprintf('SVRG 2nd: Epoch = %03d, cost = %.24e, optgap = %.4e\n', epoch, f_val, optgap);
        end
    end
